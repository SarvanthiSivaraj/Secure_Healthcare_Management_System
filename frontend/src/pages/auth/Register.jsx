...
function Register() {
    ...
    const handleVerifyOTP = async (e) => {
        ...
        try {
            let email = '';
            let password = '';
            if (activeTab === 'patient') { email = patientData.email; password = patientData.password; }
            if (activeTab === 'doctor') { email = doctorData.email; password = doctorData.password; }
            if (activeTab === 'hospital') { email = hospitalData.email; password = hospitalData.password; }
    ...
    const handlePasskeySetup = async () => {
        ...
        try {
            ...
            try {
                ...
                // Step 2: Trigger browser's WebAuthn prompt
                ...
                if (prevToken) {
                    localStorage.setItem('healthcare_token', prevToken);
                } else {
            ...
            if (err.name === 'NotAllowedError') {
                ...
                setError(err.response?.data?.message || err.message || 'Passkey setup failed');
            }
        } finally {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-dark-bg p-4 transition-colors duration-300 relative overflow-hidden">
                ...
                <div className="w-full max-w-md bg-white dark:bg-dark-card rounded-3xl shadow-card dark:shadow-none p-8 z-10 animate-fade-in text-center">
                    ...
                    <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Account Verified!</h2>
                    <p className="text-gray-500 dark:text-gray-400 mb-8">Would you like to set up a passkey for faster, passwordless sign-in?</p>

                    <div className="p-4 rounded-xl bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-700/50 mb-6">
                        <div className="flex items-center justify-center space-x-4 text-sm text-primary-700 dark:text-primary-300">
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-xs">PIN/Pattern</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        <FormButton loading={passkeyLoading} onClick={handlePasskeySetup} type="button">
                            Set Up Passkey
                        </FormButton>
                        <button
                            type="button"
                            onClick={() => navigate('/login', { state: { message: 'Registration successful! Please login.' } })}
                            className="w-full text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 underline py-2"
                        >
                            Skip for now
                        </button>
                    </div>
