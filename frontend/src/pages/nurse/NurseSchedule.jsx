...
const NurseSchedule = () => {
    ...
    useEffect(() => {
        const fetchSchedule = async () => {
            try {
                const response = await nurseApi.getSchedule();
                if (response.success) {
        ...
        <div className="flex h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden font-['Inter']">
            ...
            <aside className="w-72 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-slate-200 dark:border-slate-800 p-8 flex flex-col h-full z-20">
                ...
                <nav className="flex-grow space-y-2">
                    ...
                    <Link to="/nurse/vitals" className="flex items-center gap-3 px-4 py-3 rounded-xl transition text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
                    ...
                    <Link to="/nurse/schedule" className="flex items-center gap-3 px-4 py-3 rounded-xl transition bg-teal-50/80 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 border border-teal-100 dark:border-teal-800/50 shadow-sm font-medium">
            ...
            <main className="flex-grow overflow-y-auto bg-slate-50 dark:bg-slate-950 p-10 scrollbar-hide relative">
                <header className="mb-10">
                    <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Shift Schedule</h2>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Your clinical assignments and duty roster.</p>
                </header>
                ...
                <div className="max-w-4xl">
                    {loading ? (
                        ...
                        <div className="space-y-8">
                            <div className="bg-white/70 dark:bg-slate-900/40 backdrop-blur-md p-8 rounded-[2rem] border border-slate-200/50 dark:border-slate-800/50 relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-4 mb-8">
                                    ...
                                    <div>
                                        <h3 className="text-2xl font-bold">Duty Details</h3>
                                        <p className="text-slate-500 dark:text-slate-400 font-medium">{schedule.organizationName}</p>
                                    </div>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-slate-50 dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-100 dark:border-slate-800">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Shift Timing</p>
                                        <p className="text-xl font-bold flex items-center gap-2">
                                            {schedule.shift_start} — {schedule.shift_end}
                                            <span className="text-xs bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 px-2 py-0.5 rounded-full">Current Shift</span>
                                        </p>
                                    </div>
                                    <div className="bg-slate-50 dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-100 dark:border-slate-800">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Shift Preference</p>
                                        <p className="text-xl font-bold">{schedule.shiftPreference || 'Day Shift'}</p>
                                    </div>
                                </div>

                                <div className="mt-8">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Assigned Wards / Areas</p>
                                    <div className="flex flex-wrap gap-3">
                                        {schedule.assignedWards && schedule.assignedWards.length > 0 ? (
                                            schedule.assignedWards.map((ward, i) => (
                                                <div key={i} className="flex items-center gap-2 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 px-4 py-2 rounded-xl border border-teal-100/50 dark:border-teal-800/20 font-bold text-sm">
                                                    <span className="material-symbols-outlined text-sm">domain</span>
                                                    {ward}
                                                </div>
                                            ))
                                        ) : (
                                            <div className="text-slate-400 italic">Default assignment: General Ward</div>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="text-center py-32 bg-white/40 dark:bg-slate-900/20 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                            ...
                            <p className="text-slate-500 font-medium">No assigned schedule found in current mapping.</p>
                        </div>
                    )}
                </div>
            </main>
